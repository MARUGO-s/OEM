    ...
    <div className="help-page">
      ...
      <div className="help-steps">
        {[
          ...
          {
            title: "会議資料を添付して保管",
            text: "新しい会議の「会議の添付資料」からExcel・Word・PDF・PowerPoint・CSV・TXTを追加できます。資料は録音とは別枠で最大5ファイル・各10 MB・合計25 MB。既存会議では「添付資料」から追加できます。資料は会議の参考資料として保存するだけで、AIには送信せず、議事録の解析にも使いません。保存した資料は「添付資料」からいつでもダウンロードできます。",
          },
          {
      ...
      <section className="help-note">
        ...
        <div>
          ...
          <p>
            {isCloud
              ? "会議・音声・資料はSupabaseの会議録専用領域に非公開で保存し、ログインした全員で共有します。追加・編集・削除も共通です。他の人の変更は約10秒ごとに反映します（編集中を除く）。OpenAIとGeminiのAPIキーはワークスペース共通で別々に暗号化保存し、画面には再表示しません。Gemini文字起こしを選ぶと音声をGoogleへ送信し、文字起こし後に一時ファイルの削除を要求します。テキスト・議事録生成はOpenAIへ送信され、各APIの利用料がかかります。添付資料はAIへ送信しません。会議の削除・資料の関連付け解除後も資料は保持します。復元・完全消去は管理者にご依頼ください。"
              : "会議・音声・資料はアプリの .data フォルダに保存されます。Dropboxの設定によってはクラウドにも同期されます。Gemini文字起こしを選ぶと音声をGoogleへ送信し、テキスト・議事録生成はOpenAIへ送信します。各APIの利用料がかかります。添付資料はAIへ送信しません。関連付けを解除した資料も保持します。削除した会議は .data/trash に移動します。"}
          </p>
        </div>
      </section>
      <section className="help-note">
