...
- AAC / MP3 / M4A / WAV / MP4 / MPEG / MPGA / WebM / OGG / FLAC。1会議は最大5ファイル・合計24 MBまで。異なる形式の混在にも対応します。
- AAC（ADTS、またはAAC入りMP4）はMediabunnyで再圧縮せずM4Aへ変換して保存・送信します。表示は元のファイル名です。破損・途中で形式が変わるAAC・ADIF/LATMなど非対応形式はエラーにします。変換後も各ファイル24 MB以下、AACは25万フレームまでです。
...
- 文字起こしは最大5本を並列に処理し、完了した録音ごとに保存します。一部が失敗したら「再生成」で未完了分のみ再試行します。全録音が揃うまでは議事録を生成しません。既存の単一ファイル会議も引き続き使えます。
- 自動圧縮・自動分割は未実装です。合計24 MBを超える場合は、事前に圧縮するか別の会議に分けてください。クラウドの文字起こし待機上限は1ファイル110秒です。
