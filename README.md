...
3. 録音ファイル、その場での録音、または文字起こし済みのトークを取り込みます。途切れて複数ファイルになった録音は、まとめて選び、上下ボタンで録音順に並べてください。ファイルの追加・解除もできます。会議名、開催日、参加者、議事録の詳しさを指定できます。
   - 「その場で録音」では、ブラウザーのマイクで直接録音できます。PCのChrome/Edgeでは「マイク＋会議の音声」を選ぶと、画面共有ダイアログで会議のタブを選び「タブの音声を共有」にチェックすることで、オンライン会議の相手の声も録音できます。Safari・Firefox・スマートフォンではマイクのみの録音です。
