...
- 新しい会議の「会議の添付資料」で、資料をドラッグ＆ドロップするかボタンから選択します。録音・会話テキストと一緒に選び、音声・全資料の保存が完了してから解析します。資料だけでは会議を作成できません。
- 既存の会議にも「添付資料」へドラッグ＆ドロップまたはボタンで追加して保存できます。「議事録を再生成」を押すと、現在の全資料と保存済みの文字起こしを照合します。追加・解除後の議事録は未反映と表示します。解析中は資料を変更できません。
