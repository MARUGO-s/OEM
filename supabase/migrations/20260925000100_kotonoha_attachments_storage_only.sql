-- Attachments are stored for reference only and no longer affect the minutes,
-- so adding or unlinking one must not mark the minutes stale.
-- Replaces the dedicated kotonoha RPC only; no shared Auth/schema/policy changes.
create or replace function public.kotonoha_attachments(p_operation text,p_owner uuid,p_id uuid,p_payload jsonb default '{}'::jsonb)
...
begin
  ...
  update kotonoha.meetings set document=document || jsonb_build_object('attachments',files), updated_at=now() where id=p_id;
